...
ANSC_STATUS ssp_create ()
{
    /* Create component common data model object */
     ...
     g_pComponent_COMMON_gponmanager = (PCOMPONENT_COMMON_GPON_MANAGER)AnscAllocateMemory(sizeof(COMPONENT_COMMON_GPON_MANAGER));

    if ( ! g_pComponent_COMMON_gponmanager )
    {
        return ANSC_STATUS_RESOURCES;
    }

    ComponentCommonDmInit( g_pComponent_COMMON_gponmanager);

     g_pComponent_COMMON_gponmanager->Name     = AnscCloneString(COMPONENT_NAME_GPON_MANAGER);
     g_pComponent_COMMON_gponmanager->Version  = 1;
     g_pComponent_COMMON_gponmanager->Author   = AnscCloneString("Your name");
    ...
    if ( !pDslhLcbIf )
    {
        ...
        else
        {
            ...
            pDslhLcbIf->InitLibrary              = GponMgrDml_Init;
        }
    }
...
ANSC_STATUS ssp_engage ()
{
    ...
    char                            CrName[256];
     ...
     g_pComponent_COMMON_gponmanager->Health = COMMON_COMPONENT_HEALTH_Yellow;
    ...
    if ( returnStatus == ANSC_STATUS_SUCCESS )
    {
        /* System is fully initialized */
         g_pComponent_COMMON_gponmanager->Health = COMMON_COMPONENT_HEALTH_Green;
    }
...
ANSC_STATUS ssp_cancel ()
{
    ...
    if(  g_pComponent_COMMON_gponmanager == NULL)
    {
        return ANSC_STATUS_SUCCESS;
    }
    ...
    {
        _ansc_sprintf(CrName, "%s%s", g_Subsystem, CCSP_DBUS_INTERFACE_CR);
        _ansc_sprintf(CpName, "%s%s", g_Subsystem, COMPONENT_NAME_GPON_MANAGER);
    }
    else
    {
        _ansc_sprintf(CrName, "%s", CCSP_DBUS_INTERFACE_CR);
        _ansc_sprintf(CpName, "%s", COMPONENT_NAME_GPON_MANAGER);
    }
    ...
    if (  g_pComponent_COMMON_gponmanager ) AnscFreeMemory( g_pComponent_COMMON_gponmanager);

     g_pComponent_COMMON_gponmanager = NULL;
    pSsdCcdIf                = NULL;
...
{
    return  g_pComponent_COMMON_gponmanager->Name;
}

ULONG ssp_CcdIfGetComponentVersion (ANSC_HANDLE hThisObject)
{
    return  g_pComponent_COMMON_gponmanager->Version;
}

char* ssp_CcdIfGetComponentAuthor (ANSC_HANDLE hThisObject)
{
    return  g_pComponent_COMMON_gponmanager->Author;
}

ULONG ssp_CcdIfGetComponentHealth (ANSC_HANDLE hThisObject)
{
    return  g_pComponent_COMMON_gponmanager->Health;
}

ULONG ssp_CcdIfGetComponentState (ANSC_HANDLE hThisObject)
{
    return  g_pComponent_COMMON_gponmanager->State;
}

BOOL ssp_CcdIfGetLoggingEnabled (ANSC_HANDLE hThisObject)
{
    return  g_pComponent_COMMON_gponmanager->LogEnable;
}

ANSC_STATUS ssp_CcdIfSetLoggingEnabled (ANSC_HANDLE hThisObject,BOOL bEnabled)
{
    if( g_pComponent_COMMON_gponmanager->LogEnable == bEnabled) return ANSC_STATUS_SUCCESS;
     g_pComponent_COMMON_gponmanager->LogEnable = bEnabled;
    if(bEnabled) g_iTraceLevel = (INT)  g_pComponent_COMMON_gponmanager->LogLevel;
...
{
    return  g_pComponent_COMMON_gponmanager->LogLevel;
}

ANSC_STATUS ssp_CcdIfSetLoggingLevel (ANSC_HANDLE hThisObject,ULONG LogLevel)
{
    if( g_pComponent_COMMON_gponmanager->LogLevel == LogLevel) return ANSC_STATUS_SUCCESS;
     g_pComponent_COMMON_gponmanager->LogLevel = LogLevel;
    if( g_pComponent_COMMON_gponmanager->LogEnable) g_iTraceLevel = (INT)  g_pComponent_COMMON_gponmanager->LogLevel;
...
{
    return  g_pComponent_COMMON_gponmanager->MemMinUsage;
}
...
ULONG ssp_CcdIfGetMemConsumed (ANSC_HANDLE hThisObject)
{
    ...
    size = AnscGetComponentMemorySize(COMPONENT_NAME_GPON_MANAGER);
    if (size < 0 )
        size = 0;
...
ANSC_STATUS ssp_CcdIfApplyChanges (ANSC_HANDLE hThisObject)
{
    ...
    /* AnscSetTraceLevel((INT) g_pComponent_COMMON_gponmanager->LogLevel); */
