...
#define  COMPONENT_ID_GPON_MANAGER                             "com.cisco.spvtg.ccsp.gponmanager"
#define  COMPONENT_NAME_GPON_MANAGER                           "com.cisco.spvtg.ccsp.gponmanager"
#define  COMPONENT_VERSION_GPON_MANAGER                        1
#define  COMPONENT_PATH_GPON_MANAGER                           "/com/cisco/spvtg/ccsp/gponmanager"

#define  MESSAGE_BUS_CONFIG_FILE                            "msg_daemon.cfg"

typedef  struct
_COMPONENT_COMMON_GPON_MANAGER
{
    char*                           Name;
    ULONG                           Version;
    char*                           Author;
...
COMPONENT_COMMON_GPON_MANAGER,  *PCOMPONENT_COMMON_GPON_MANAGER;

#define ComponentCommonDmInit(component_com_gponmanager)                                          \
        {                                                                                  \
            AnscZeroMemory(component_com_gponmanager, sizeof(COMPONENT_COMMON_GPON_MANAGER));             \
            component_com_gponmanager->Name        = NULL;                                        \
            component_com_gponmanager->Version     = 1;                                           \
            component_com_gponmanager->Author      = NULL;                                        \
            component_com_gponmanager->Health      = COMMON_COMPONENT_HEALTH_Red;            \
            component_com_gponmanager->State       = COMMON_COMPONENT_STATE_Running;         \
            if(g_iTraceLevel >= CCSP_TRACE_LEVEL_EMERGENCY)                                \
                component_com_gponmanager->LogLevel = (ULONG) g_iTraceLevel;                      \
            component_com_gponmanager->LogEnable   = TRUE;                                        \
            component_com_gponmanager->MemMaxUsage = 0;                                           \
            component_com_gponmanager->MemMinUsage = 0;                                           \
            component_com_gponmanager->MemConsumed = 0;                                           \
        }


#define  ComponentCommonDmClean(component_com_gponmanager)                                        \
         {                                                                                  \
            if ( component_com_gponmanager->Name )                                                \
            {                                                                               \
                AnscFreeMemory(component_com_gponmanager->Name);                                  \
            }                                                                               \
                                                                                            \
            if ( component_com_gponmanager->Author )                                              \
            {                                                                               \
                AnscFreeMemory(component_com_gponmanager->Author);                                \
            }                                                                               \
         }


#define  ComponentCommonDmFree(component_com_gponmanager)                                         \
         {                                                                                  \
            ComponentCommonDmClean(component_com_gponmanager);                                    \
            AnscFreeMemory(component_com_gponmanager);                                            \
         }
